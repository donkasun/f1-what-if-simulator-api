...
class SimulationService:
    ...
    def _prepare_features(
        ...
        # TODO: FWI-BE-111 - Update ML model to include speed data features
        features = [
            request.driver_id,
            request.track_id,
            request.season,
    ...
    def _calculate_confidence_score(self, historical_data: dict) -> float:
        ...
        total_laps = historical_data.get("total_laps", 0)
        consistency_score = historical_data.get("consistency_score", 0.0)

        # Base confidence on number of laps and consistency
        if total_laps >= 50:
        ...
        return min(0.95, base_confidence * (0.7 + 0.3 * consistency_score))
